...
/*!******************************!*\
  ...
  \******************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _form_scss__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./form.scss */ "./src/frontend/form.scss");
/**
...
(function () {
  ...
  function serialize(form) {
    ...
    elements.forEach(function (el) {
      ...
      if (el.type === 'select-multiple') {
        ...
        data[msKey] = Array.prototype.map.call(el.selectedOptions, function (o) {
          return o.value;
        });
  ...
  function clearErrors(form) {
    form.querySelectorAll('.activeforms-error').forEach(function (e) {
      e.textContent = '';
    });
    form.querySelectorAll('.activeforms-field--invalid').forEach(function (e) {
      e.classList.remove('activeforms-field--invalid');
    });
  }
    ...
    form.addEventListener('submit', function (event) {
      ...
      fetch(cfg.restUrl, {
        ...
        if (res.success) {
          ...
          if (msg) {
            msg.className = 'activeforms-form-message activeforms-form-message--success';
            msg.innerHTML = html;
          }
        } else {
          if (res.errors) showErrors(form, res.errors);
          if (msg) {
            msg.className = 'activeforms-form-message activeforms-form-message--error';
            msg.textContent = res.message || 'Please check the form.';
          }
        }
      }).catch(function () {
        if (btn) {
  ...
  function efFormat(d, format) {
    ...
    var map = {
      Y: d.getFullYear(),
      y: efPad(d.getFullYear() % 100),
      m: efPad(d.getMonth() + 1),
      n: d.getMonth() + 1,
      d: efPad(d.getDate()),
      j: d.getDate(),
      M: EF_SHORT[d.getMonth()],
      H: efPad(H),
      h: efPad(h12),
      i: efPad(d.getMinutes()),
      K: H < 12 ? 'AM' : 'PM'
    };
    var out = '';
  ...
  function datePicker(input) {
    ...
    function inRange(d) {
      return !!(isRange && start && end && d > stripTime(start) && d < stripTime(end));
    }
    ...
    function commit(close) {
      ...
      if (mode === 'time') {
        ...
        if (start && end) {
          val += ' to ' + efFormat(end, format);
        }
      } else if (mode === 'datetime') {
        if (start) {
          val = efFormat(withTime(start), format);
        }
      } else if (start) {
        val = efFormat(start, format);
      }
    ...
    function nav(txt, fn) {
      var b = el('button', 'activeforms-dp__nav', txt);
      b.type = 'button';
    ...
    function mkSelect(min, max, val, onCh) {
      ...
      for (var i = min; i <= max; i++) {
        var o = el('option', '', efPad(i));
        o.value = i;
        s.appendChild(o);
      }
      s.value = val;
      s.addEventListener('change', function () {
        onCh(parseInt(s.value, 10));
      });
      return s;
    }
    function buildCalendar() {
      ...
      EF_DOW.forEach(function (w) {
        dow.appendChild(el('span', '', w));
      });
      cal.appendChild(dow);
    ...
    function buildTime() {
      ...
      t.appendChild(mkSelect(is24 ? 0 : 1, is24 ? 23 : 12, hour, function (v) {
        hour = v;
        commit(false);
      }));
      t.appendChild(el('span', 'activeforms-dp__colon', ':'));
      t.appendChild(mkSelect(0, 59, minute, function (v) {
        minute = v;
        commit(false);
      }));
      if (!is24) {
    ...
    function build() {
      pop.innerHTML = '';
      if (hasCal) {
        pop.appendChild(buildCalendar());
      }
      if (hasTime) {
        pop.appendChild(buildTime());
      }
