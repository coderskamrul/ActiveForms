...
function ConfigModal({ integration, onClose, onSaved }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.45)', zIndex: 1200, display: 'grid', placeItems: 'center' }} onClick={onClose}>
      <div className="easyforms-card" style={{ width: 460, maxWidth: '92vw' }} onClick={(e) => e.stopPropagation()}>
...
export default function Integrations() {
  ...
  return (
    <div>
      <PageHead title="Integrations" subtitle="Connect EasyForms to your favorite tools" />
      {Object.entries(grouped).map(([cat, items]) => (
        <div key={cat} style={{ marginBottom: 22 }}>
          <h3 style={{ marginBottom: 10 }}>{CAT_LABELS[cat] || cat}</h3>
          <div className="easyforms-grid easyforms-grid-4">
            {items.map((i) => (
              <Card key={i.slug} pad={false}>
                <div className="easyforms-card-pad">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <strong>{i.title}</strong>
                    {i.configured && <span className="easyforms-badge easyforms-badge--published">Ready</span>}
                  </div>
      ...
      <Card>
        <p style={{ margin: 0, color: 'var(--_muted)' }}>
          40+ more integrations (HubSpot, Salesforce, ActiveCampaign, Zapier, Google Sheets, Twilio, and more) are available in EasyForms Pro.
        </p>
      </Card>
