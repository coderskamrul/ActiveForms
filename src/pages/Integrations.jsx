...
function ConfigModal({ integration, onClose, onSaved }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.45)', zIndex: 1200, display: 'grid', placeItems: 'center' }} onClick={onClose}>
      <div className="radiusforms-card" style={{ width: 460, maxWidth: '92vw' }} onClick={(e) => e.stopPropagation()}>
        <div className="radiusforms-card-pad">
...
export default function Integrations() {
  ...
  return (
    <div>
      <PageHead title="Integrations" subtitle="Connect RadiusForms to your favorite tools" />
      {Object.entries(grouped).map(([cat, items]) => (
        <div key={cat} style={{ marginBottom: 22 }}>
          <h3 style={{ marginBottom: 10 }}>{CAT_LABELS[cat] || cat}</h3>
          <div className="radiusforms-grid radiusforms-grid-4">
            {items.map((i) => (
              <Card key={i.slug} pad={false}>
                <div className="radiusforms-card-pad">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <strong>{i.title}</strong>
                    {i.configured && <span className="radiusforms-badge radiusforms-badge--published">Ready</span>}
                  </div>
