...
function SubControl({ sub }) {
  ...
  const lpClass = ` radiusforms-subfield--lp-${lp}`;
  return (
    <div className={`radiusforms-subfield${lpClass}`} style={{ flex: '1 1 0', minWidth: 0 }}>
      {sub.type === 'country' ? (
...
export default function FieldPreview({ field }) {
  ...
  if (field.type === 'submit') {
    ...
    return (
      <div style={{ display: 'flex', justifyContent: justify }}>
        <span className="radiusforms-btn radiusforms-btn--primary">{field.label || 'Submit'}</span>
      </div>
    );
  }
  ...
  return (
    <div className={`radiusforms-mock radiusforms-mock--lp-${lp}`}>
      {lp !== 'hide' && (
