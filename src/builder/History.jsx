...
export default function History({ canUndo, canRedo, onUndo, onRedo, depth, lastSaved }) {
  return (
    <div className="easyforms-hist">
      <div className="easyforms-hist__row">
        <button type="button" className="easyforms-btn" disabled={!canUndo} onClick={onUndo}>
          <Icon name="undo" size={15} /> Undo
        </button>
        <button type="button" className="easyforms-btn" disabled={!canRedo} onClick={onRedo}>
          <Icon name="redo" size={15} /> Redo
        </button>
      </div>

      <div className="easyforms-hist__meta">
        <div className="easyforms-hist__stat">
          <span className="num">{depth}</span>
          <span className="lbl">undoable change{depth === 1 ? '' : 's'} this session</span>
        </div>
      </div>

      <div className="easyforms-hist__card">
        <div className="easyforms-hist__card-head"><Icon name="history" size={15} /> Saved version</div>
        {lastSaved
