...
class FormRenderer {
	...
	public function render( $form ) {
		...
		$nonce  = wp_create_nonce( 'radiusforms_submit_' . $id );

		$html  = '<div class="radiusforms-form-wrap" data-form-id="' . esc_attr( $id ) . '">';
		$html .= '<form class="radiusforms-form" method="post" data-form-id="' . esc_attr( $id ) . '" novalidate>';
		$html .= '<input type="hidden" name="radiusforms_form_id" value="' . esc_attr( $id ) . '" />';
		$html .= '<input type="hidden" name="radiusforms_nonce" value="' . esc_attr( $nonce ) . '" />';

		$guard = new SpamGuard();
		$html .= $guard->honeypot_markup();

		$html .= '<div class="radiusforms-fields">';
		$html .= $this->render_fields( $schema );
		$html .= '</div>';

		if ( ! $this->has_submit( $schema ) ) {
			$html .= '<div class="radiusforms-submit radiusforms-submit--left"><button type="submit" class="radiusforms-btn radiusforms-btn--primary">'
				. esc_html__( 'Submit', 'radiusforms' ) . '</button></div>';
		}

		$html .= '<div class="radiusforms-form-message" role="status" aria-live="polite"></div>';
		$html .= '</form>';
		$html .= '</div>';
		 ...
		 */
		return apply_filters( 'radiusforms/rendering_form', $html, $form );
	}
	...
	protected function render_container( $field ) {
		...
		$html    = '<div class="radiusforms-row radiusforms-row--cols-' . esc_attr( $count ) . '" style="display:flex;flex-wrap:wrap;gap:20px;">';
		...
		foreach ( $columns as $column ) {
			...
			$html .= '<div class="radiusforms-col" style="flex:' . esc_attr( max( 1, $width ) ) . ' 1 0;min-width:0;">';
