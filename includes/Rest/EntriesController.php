...
class EntriesController extends AbstractController {
	...
	public function show( $request ) {
		...
		if ( ! $entry ) {
			return $this->fail( __( 'Entry not found.', 'radiusforms' ), 404 );
		}
	...
	protected function resolve_user( $user_id ) {
		...
		if ( ! $user ) {
			return array(
				'id'        => $user_id,
				'name'      => __( 'Deleted user', 'radiusforms' ),
				'edit_link' => '',
			);
		}
	...
	public function bulk( $request ) {
		...
		if ( empty( $ids ) ) {
			return $this->fail( __( 'No entries selected.', 'radiusforms' ), 422 );
		}
		...
		foreach ( $ids as $id ) {
			...
			switch ( $action ) {
				...
				case 'unfavorite':
					...
					break;
				default:
					return $this->fail( __( 'Unknown bulk action.', 'radiusforms' ), 422 );
			}
			++$affected;
		}
