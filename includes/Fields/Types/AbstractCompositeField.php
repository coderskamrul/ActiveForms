...
abstract class AbstractCompositeField extends AbstractField {
	...
	public function validate( $value, $field ) {
		...
		foreach ( $this->subfields( $field ) as $sub ) {
			...
			if ( '' === trim( (string) Arr::get( $value, $sub['key'], '' ) ) ) {
				...
				return sprintf( __( '%s is required.', 'easyforms' ), $sub['label'] );
			}
		}
		return true;
	}
	...
	protected function subfield_control( $field, $sub, $value ) {
		...
		$id    = 'easyforms-' . $key . '-' . esc_attr( $sub['key'] );
		...
		$lp_class  = ' easyforms-subfield--lp-' . sanitize_html_class( $placement );

		$html = '<div class="easyforms-subfield' . $lp_class . '">';

		if ( 'country' === Arr::get( $sub, 'type', 'text' ) ) {
			$searchable = ! empty( $sub['searchable'] ) ? ' data-easyforms-searchable="1"' : '';
			$html .= '<select class="easyforms-input easyforms-select easyforms-country" name="' . $name . '" id="' . $id . '"' . $req . $searchable . '>';
			$html .= '<option value="">' . esc_html__( '— Select Country —', 'easyforms' ) . '</option>';
			...
			$html .= sprintf(
				'<input type="text" class="easyforms-input" name="%1$s" id="%2$s" placeholder="%3$s" value="%4$s"%5$s />',
				$name,
				$id,
				esc_attr( $plabel ),
				esc_attr( $pval ),
		...
		if ( 'hide' !== $placement ) {
			$html .= '<small class="easyforms-sublabel">' . esc_html( $sub['label'] );
